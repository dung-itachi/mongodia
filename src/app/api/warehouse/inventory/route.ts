...
export async function GET(request: Request) {
  try {
    ...
    if (!warehouseId && !canAccessAllWarehouses(currentUser)) return error("Nhân viên kho chỉ được xem tồn kho kho được giao", 403);
    if (warehouseId && !canAccessWarehouse(currentUser, warehouseId)) return error("Bạn không có quyền truy cập kho này", 403);
    await connectDB();
    return success(await warehouseWorkflowService.listInventory({
      warehouseId,
      itemType: params.get("itemType") ?? undefined,
      page: Number(params.get("page") ?? 1),
      limit: Math.min(Number(params.get("limit") ?? 20), 100),
    }));
  } catch (cause) { return error(cause instanceof Error ? cause.message : "Không thể lấy tồn kho", 400); }
}
