...
export async function GET(request: Request) {
  try {
    ...
    const chartData = await salesKPIService.getKPIChart(employeeId, month, year);

    return NextResponse.json(chartData);
  } catch (error) {
    console.error("[SALES KPI CHART API]", error);
    return serverError("Không thể lấy dữ liệu chart KPI");
  }
}
